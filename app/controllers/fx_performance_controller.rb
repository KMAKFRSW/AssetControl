...
class FxPerformanceController < ApplicationController
  def index
    ...
    @tryjpy_term_risk_graph = term_risk_graph('TRY/JPY','トルコリラ円')
    ...
    @tryjpy_avg_daily_range_graph = avg_daily_range_graph('TRY/JPY','トルコリラ円', 0, 10)
    ...
    @tryjpy_dfma_graph = dfma_graph('TRY/JPY','トルコリラ円')
    ...
    @tryjpy_rsi_and_st_graph = rsi_and_st_graph('TRY/JPY','トルコリラ円')
  ...
  #      f.series(:type => 'line', name: '移動平均線(100 day)'   , data: range_100d_avg  , pointFormat: '移動平均線(100 day):')
    ...
    def dfma_graph(cur_code,cur_name)
      ...
      @dfma.each do |dfma|
        ...
        dfma_100d.push(dfma.DFMA100.to_f)
      end
