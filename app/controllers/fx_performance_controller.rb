...
class FxPerformanceController < ApplicationController
  def index
    ...
    @usdjpy_term_risk_graph = LazyHighCharts::HighChart.new('graph') do |f|
      ...
      f.yAxis(:title => {:text => 'USD/JPY Historical Volatility'}, :min => 0, :max => 1, tickInterval: 0.02 )
    ...
    @eurjpy_term_risk_graph = LazyHighCharts::HighChart.new('graph') do |f|
      ...
      f.yAxis(:title => {:text => 'EUR/JPY Historical Volatility'}, :min => 0, :max => 1, tickInterval: 0.02 )
    ...
    @usdeur_term_risk_graph = LazyHighCharts::HighChart.new('graph') do |f|
      ...
      f.yAxis(:title => {:text => 'USD/EUR Historical Volatility'}, :min => 0, :max => 1, tickInterval: 0.02 )
